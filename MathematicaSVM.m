...
loadData[filename_]:=Module[{dtst,feats,labels,training,testing,fTe,yTe,fTr,yTr},
...
loadData[x___/;(Length[{x}]==0||Length[{x}]>1)]:=Message[loadData::badarg,{x}]
...
hinge[margin_]:=Piecewise[{{1-margin,(1-margin)>0}},0];
(*hinge[margin_]:=Piecewise[{{1-margin,margin<1}},0];*)

err[margin_]:=Piecewise[{{1,margin<0}},0];
...
trainMaxMargin[fTr_,yTr_]:=Module[{results,model,margin,nTr,fTr2,d,w,v,b,i,sol,cnstr},
{nTr,d}=Dimensions[fTr];
...
trainSoftMargin[fTr_List,yTr_List,regC_]:=Module[{results,model,margin,nTr,fTr2,d,w,v,b,xi,x,i,sol,obj,cnstr},
{nTr,d}=Dimensions[fTr];
w=Table[Subscript[v, i],{i,d}];
...
trainSoftMarginHinge[feats_List,labels_List,regC_]:=Module[{results,model,margin,b,d,nTr,v,w,regularizer,loss,obj,sol},
...
trainZeroOneError[feats_List,labels_List,regC_]:=Module[{results,model,margin,b,d,nTr,v,w,regularizer,loss,obj,sol},
{nTr,d}=Dimensions[feats];
w=Table[Subscript[v, i],{i,1,d}];
regularizer=w.w;
...
trainSoftMarginHingeWbias[feats_List,labels_List,lambda_,function_]:=Module[{model,margin,results,biasFeats,d,nTr,w,v,regularizer,loss,obj,sol},
...
trainZeroOneErrorWbias[feats_List,labels_List,lambda_]:=Module[{model,margin,results,biasFeats,d,nTr,w,v,regularizer,loss,obj,sol},
biasFeats=Append[#,1]& /@feats;
{nTr,d}=Dimensions[biasFeats];
w=Table[Subscript[v, i],{i,1,d}];
regularizer[u_]:=lambda/2 Norm[u]^2;
...
trainHardMarginSVM[KTr_,yTr_]:=Module[{nTr,d,H,f,a,alpha,b,margin,sol,obj,constraints},
...
train1NormSoftMarginSVM[KTr_,yTr_,regC_]:=Module[{nTr,d,H,f,a,alpha,b,nrm,margin,sol,obj,constraints},
{nTr,d}=Dimensions[KTr];
f=Table[1,{i,nTr}];
alpha=Table[Subscript[a, i],{i,nTr}];
H=yTr.Transpose[yTr] KTr;
...
testLinearClassifier[w_,feats_,labels_]:=Module[{biasFeats,nTe,d,pred,acc,score},
biasFeats=Append[#,1]& /@feats;
...
testSVM[model_,KTe_,yTe_]:=Module[{nTe,nTr,a,b,pred,score,correct},
a=model[[1]];
b=model[[2]];
...
plotData[feats_List,labels_List]:=Module [{pos,neg},
...
plotLinearResults[testFunc_,model_,feats_List,labels_List,trErr_Real,teErr_Real,margin_Real]:=Module [{pos,neg,minP,maxP,a,b,c},
pos=Transpose[MatrixForm[Position[labels,1.]]][[All,1]][[1]];
neg=Transpose[MatrixForm[Position[labels,-1.]]][[All,1]][[1]];
...
plotLinearResults[testFunc_,model_,fTr_List,yTr_List,fTe_List,yTe_List,trErr_Real,teErr_Real,margin_Real]:=Module [{f1,f2,posTr,negTr,posTe,negTe,minP,maxP,a,b,c},
...
plotKernelResults[model_,fTr_,yTr_,fTe_,yTe_,kernelFunc_,trErr_,teErr_,margin_]:=Module [{sv,posTe,negTe,pos,posSV,negSV,posAll,negAll,fnSV,fpSV,neg,minP,maxP,a,b,c,d},
...
runMaxMarginExperiment[fTr_List,yTr_List,fTe_List,yTe_List,classifier_]:=Module[{model,margin,teResults,trResults},
...
runNoBiasExperiment[fTr_List,yTr_List,fTe_List,yTe_List,classifier_]:=Module[{model,margin,lambda,trResults,teResults},
...
runSVMExperiment[fTr_List,yTr_List,fTe_List,yTe_List,classifier_,kernFunc_]:=Module[{KTr,model,margin,nTr,trResults,teResults},
...
Protect[plotSize,loadData,createData,getTrTeData,hinge,err,
linearKernel,gaussianKernel,computeGaussianKernel,estimateSigmaSQ,computeDist,
trainMaxMargin,trainSoftMargin,trainSoftMarginHinge,trainZeroOneError,
trainSoftMarginHingeWbias,trainZeroOneErrorWbias,
trainHardMarginSVM,train1NormSoftMarginSVM,train2NormSoftMarginSVM,
